...
template <typename T> inline std::ostream& debugchar(std::ostream& os, const T c) {
	return os << '"' << c << "\" (" << std::hex << static_cast<int>(c) << std::dec << ')';
}

template <typename T> inline std::string debugchar(const T c) {
...
void EGIConnection::expect(const char what) {
	...
	if (resp != what)
		std::cerr << "\nExpected " + debugchar(what) + ", got " + debugchar(resp) +
		                 "\nCode: " + static_cast<char>(socket.get())
		          << std::endl;
}
...
int64_t EGIConnection::synch() {
	sendCommand('A', 'Z');
	auto timediff = std::chrono::high_resolution_clock::now() - syncepoch;
	for (int n = 0; n < 100 && timediff > 2ms; ++n) {
		syncepoch = std::chrono::high_resolution_clock::now();
		sendRawStruct(SynchPacket(0), 'Z');
		timediff = std::chrono::high_resolution_clock::now() - syncepoch;
	}
